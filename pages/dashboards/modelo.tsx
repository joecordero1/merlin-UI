...
const Modelo = () => {
    ...
    /* ---------------- reutilizable ---------------- */
      ...
      useEffect(() => {
        Promise.all([
          fetchJSON('predictions').then(setPredictions),
          fetchJSON('total-predictions').then(setTotals),
          fetchJSON('CTR').then(setCTR),
          fetchJSON('presition').then(setPrecision),
          fetchJSON('last-updated').then(setLastUpdated)
        ]).catch(console.error);
      }, [model]);
